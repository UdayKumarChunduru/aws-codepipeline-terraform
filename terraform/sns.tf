...
resource "aws_cloudwatch_event_rule" "pipeline_events" {
  ...
  description = "All CodePipeline execution state changes → SNS"
  state       = "ENABLED"
...
resource "aws_cloudwatch_event_target" "pipeline_to_sns" {
  ...
  input_transformer {
    ...
    input_template = <<-EOT
      ...
      "Run ID:   <execution>"
      "Time:     <time>"
      "Console:  https://console.aws.amazon.com/codesuite/codepipeline/pipelines/<pipeline>/executions/<execution>/visualization?region=<region>"
    EOT
  }
}
